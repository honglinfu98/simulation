...
$0.28$--$9.1$): a protocol-matched reference, not the published model's
attainable accuracy.
...
broad competitiveness, not dominance. The F2 panel separates the
...
falsifiable: probes must bracket and converge to the validation target, a
full-scale roll-out must reproduce it within 15\%, and failures are
reproducible (deterministic seeds, bit-identical retry brackets).
...
softmin bound in every reported run --- an empirical regularity, not a
theorem.
