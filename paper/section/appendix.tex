...
\subsection{The architectural rate bound}
...
$\lambda<\varsigma\,\mathrm{softplus}(z_{\max})$. 

(i) The conditional intensity is globally bounded,
...
$1.102$ (s3); \spppub{} (passing) BTC $0.608$ (s1), $0.439$ (s2), ETH
$0.322/0.310/0.439$, SOL $0.534$ (s2).
...
\subsection{Stylized-fact construction (order-flow proxy)}
...
compensator-inversion sampling for baselines. The \spppub{} port sizes are
benchmark-matched ($H{=}64$, two layers, complex state dimension $P{=}64$
per layer); its architecture follows the official example configuration
(\texttt{Int\_Backward\_LLH}, \texttt{relative\_time}, post-norm, GELU) and
was verified value-level against the official classes.
