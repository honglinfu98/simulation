...
Gaps: Fano(1s) 2.99<7.9 (n=0.8→asymptotic 25; real n≈0.86 — raise `--nmh-project-rho` to ~0.9); extreme kurt 10709 unreached (needs QHawkes volatility-feedback). Code: `lgm_decoder.py` + `ptp_s2p2_decoder.py` (per-type s2p2, also usable standalone, nonlinear gauge-broken readout); factory `decoder_type='lgm'`/`'pts2p2'`; `--lgm-target-rate`, `--ptp-dim`, `--nmh-project-rho`.
