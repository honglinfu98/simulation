"""Download raw trades (cluster-only entry point).
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Download raw trades.")
    ap.add_argument("--crypto", required=True, help="symbol, e.g. wif")
    ap.add_argument("--parallel", type=int, default=4)
    ap.add_argument("--out-dir", default="data/trades")
    args = ap.parse_args()

    cred = require_credentials()
    raise SystemExit(
        f"extract.download_trades: credentials OK ({cred}), but the real "
