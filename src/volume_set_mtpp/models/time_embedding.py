...
class SinusoidalEmbedding(nn.Module):
    ...
    def forward(self, t, unused_arg=None):
        # Handle tensor shape for proper broadcasting
        # t might be [B, N] or [B, N, 1] depending on usage
        # self.weight is [embedding_dim//2]
        # unused_arg is included for compatibility with HawkesDecoder which passes two arguments
        
        if t.dim() == 2:
            # t: [B, N] → [B, N, 1] for broadcasting with weight [embedding_dim//2]
            t = t.unsqueeze(-1)
        elif t.dim() == 3 and t.size(-1) == 1:
            # t: [B, N, 1] - already in correct shape
            pass
        elif t.dim() > 3:
            # Squeeze extra dimensions - this happens when unsqueeze is called multiple times
            while t.dim() > 3:
                t = t.squeeze(-1)
            if t.dim() == 2:
                t = t.unsqueeze(-1)
        else:
            # For other cases, ensure last dim is 1
            t = t.unsqueeze(-1)
        
        # Now t: [B, N, 1], self.weight: [embedding_dim//2]
        # Broadcasting: [B, N, 1] * [embedding_dim//2] → [B, N, embedding_dim//2]
        sin_emb = torch.sin(t * self.weight)  
        cos_emb = torch.cos(t * self.weight)
        return torch.cat((sin_emb, cos_emb), dim=-1)  # [B, N, embedding_dim]
...
class TemporalEmbedding(nn.Module):
    ...
    def forward(self, t, true_times=None):
        ...
        # If t has more than 3 dimensions, squeeze extra ones
        while t.dim() > 3:
            t = t.squeeze(-1)
        if true_times.dim() > 3:
        ...
        if self.raw_time_embed is not None:
            if callable(self.raw_time_embed) and hasattr(self.raw_time_embed, '__name__'):
                # Lambda function case (embedding_dim == 1) - expects [B, N, 1]
                if t.dim() == 2:  # [B, N]
                    t_input = t.unsqueeze(-1)  # [B, N, 1]
                else:
                    t_input = t
                embeddings.append(self.raw_time_embed(t_input))
            else:
                # SinExpEmbedding case - handle dimensions properly
                embeddings.append(self.raw_time_embed(t))
        ...
        if self.delta_time_embed is not None:
            ...
            if callable(self.delta_time_embed) and hasattr(self.delta_time_embed, '__name__'):
                # Lambda function case (embedding_dim == 1)
                if delta_t.dim() == 2:  # [B, N]
                    delta_t_input = delta_t.unsqueeze(-1)  # [B, N, 1]
                else:
                    delta_t_input = delta_t
                embeddings.append(self.delta_time_embed(delta_t_input))
            else:
                # SinExpEmbedding case
                embeddings.append(self.delta_time_embed(delta_t))
