...
| `lgm_decoder.py` → `PerTypeS2P2Decoder` | Per-type ("parallel over types") s2p2 with nonlinear LayerNorm read-out. **Folded into `lgm_decoder.py` as LGM's rate-neutral mark head** (also the standalone PCT-LSTM baseline, `decoder_type 'pct-lstm'`). | `is_ptp` |
