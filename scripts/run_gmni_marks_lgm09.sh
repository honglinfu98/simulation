...
SEED=1
TAG=lgm09
BASE="experiments/gmni_marks_lgm09_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BASE"
MASTER="$BASE/master.log"
log() { echo "$@" | tee -a "$MASTER"; }
log "RUN_KIND=gmni_marks_lgm09 START $(date) HOST=$(hostname) CVD=$CUDA_VISIBLE_DEVICES"
...
echo "$BASE" > "$HOME/volume-set-mtpp/.last_lgm09_base"
exit $status
