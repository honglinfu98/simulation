...
SEED=1
TAG=nmh
BASE="experiments/gmni_marks_nmh_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BASE"
MASTER="$BASE/master.log"
log() { echo "$@" | tee -a "$MASTER"; }
log "RUN_KIND=gmni_marks_nmh START $(date) HOST=$(hostname) CVD=$CUDA_VISIBLE_DEVICES"
...
/usr/bin/time -p python3 -u -m volume_set_mtpp.training_evaluation.train_bfnx \
  ...
  --no-volume-input-scaling --mark-head categorical \
...
echo "$BASE" > "$HOME/volume-set-mtpp/.last_nmh_base"
exit $status
