...
SEED=1
TAG=lgm086
BASE="experiments/gmni_marks_lgm086_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BASE"
MASTER="$BASE/master.log"
log() { echo "$@" | tee -a "$MASTER"; }
log "RUN_KIND=gmni_marks_lgm086 START $(date) HOST=$(hostname) CVD=$CUDA_VISIBLE_DEVICES"
...
/usr/bin/time -p python3 -u -m volume_set_mtpp.training.train \
  ...
  --no-volume-input-scaling --mark-head categorical --nmh-project-rho 0.86 \
...
echo "$BASE" > "$HOME/volume-set-mtpp/.last_lgm086_base"
exit $status
