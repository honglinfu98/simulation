...
SEED=1
TAG=nmhc
BASE="experiments/gmni_marks_nmhc_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BASE"
MASTER="$BASE/master.log"
log() { echo "$@" | tee -a "$MASTER"; }
log "RUN_KIND=gmni_marks_nmhc START $(date) HOST=$(hostname) CVD=$CUDA_VISIBLE_DEVICES"
...
/usr/bin/time -p python3 -u -m volume_set_mtpp.training_evaluation.train \
  ...
  --no-volume-input-scaling --mark-head categorical --subcritical-weight 2.0 --subcritical-rho-max 0.8 \
...
echo "$BASE" > "$HOME/volume-set-mtpp/.last_nmhc_base"
exit $status
