...
# SS2P2 + LOB-STATE ABLATION on Coinbase BTC (7 days Jan 2026).
# Protocol is multi_asset_cbse.sh VERBATIM (same data, cache, epochs, seq,
# calibration, streaming prediction, equal-duration facts) so results drop
# straight into experiments/ma_cbse/btc/ next to the existing ss2p2-full-s*
# baselines. The ONLY delta is --lob-state-input: the set/volume heads are
# conditioned on the 6 continuous features derived from the top-10 book
# ladders (imbalance, log depths, log best-quote vols, spread bps); the
# stylized-facts rollout replays the book closed-loop (state_loop).
#
# Task = training seed (1-3). Compare against ss2p2-full-s{1,2,3}.
...
SEED=$SGE_TASK_ID
EXTRA="--decoder-type ss2p2 --s2p2-layers 2 --ss2p2-wnorm-cap 6.0 --target-rate -1 --tbptt --s2p2-scan --lob-state-input"
TAG="ss2p2-lob-s${SEED}"
SAMPLER=thinning
