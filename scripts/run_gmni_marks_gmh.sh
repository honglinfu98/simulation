...
SEED=1
TAG=gmh
BASE="experiments/gmni_marks_gmh_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BASE"
MASTER="$BASE/master.log"
log() { echo "$@" | tee -a "$MASTER"; }
log "RUN_KIND=gmni_marks_gmh START $(date) HOST=$(hostname) CVD=$CUDA_VISIBLE_DEVICES"
...
/usr/bin/time -p python3 -u -m volume_set_mtpp.training_evaluation.train \
  ...
  --decoder-type gmh --nmh-timescales 4 --gmh-gate-max 3.0 --s2p2-layers 3 \
...
echo "$BASE" > "$HOME/volume-set-mtpp/.last_gmh_base"
exit $status
